...
function run_database_setup() {
  mysql -h $endpoint -u admin -p$DB_PASS --execute="CREATE DATABASE bookstack;"
  mysql -h $endpoint -u admin -p$DB_PASS --execute="CREATE USER 'bookstack'@'%' IDENTIFIED BY '$DB_PASS';"
  mysql -h $endpoint -u admin -p$DB_PASS --execute="GRANT ALL PRIVILEGES ON bookstack.* TO 'bookstack'@'%';"
